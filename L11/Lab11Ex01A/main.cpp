/******************************************************************************
 * Lab 11 - Exercise 1 (version a - using file locking)                       *
...
DWORD WINAPI WorkerThread(LPVOID data)
{
    ...
    while (ReadFile(operationsFile, &operation, sizeof(LINE), &n, NULL) && n == sizeof(LINE))
    {
        ...
        if (!LockFileEx(td->accountFile, LOCKFILE_EXCLUSIVE_LOCK, 0, sizeof(LINE), 0, &ov))
        {
            _ftprintf(stderr, _T("Cannot lock accounts file.\n"));
            return -1;
        }

        _tprintf(_T("Thread %d locking from %d to %d\n"), GetCurrentThreadId(), offset.LowPart, offset.LowPart + sizeof(LINE));
        ...
        if (!ReadFile(td->accountFile, &account, sizeof(LINE), &n, &ov) || n != sizeof(LINE))
        {
            ...
            UnlockFileEx(td->accountFile, 0, sizeof(LINE), 0, &ov);
            return -1;
        }
        ...
        _tprintf(_T("Thread %d unlocking from %d to %d\n"), GetCurrentThreadId(), offset.LowPart, offset.LowPart + sizeof(LINE));
        ...
        if (!UnlockFileEx(td->accountFile, 0, sizeof(LINE), 0, &ov))
        {
            _ftprintf(stderr, _T("Cannot unlock accounts file.\n"));
            return -1;
        }
    }
