/******************************************************************************
 * Lab 11 - Exercise 1 (version c - using mutexes)                            *
...
DWORD WINAPI WorkerThread(LPVOID data)
{
    ...
    while (ReadFile(operationsFile, &operation, sizeof(LINE), &n, NULL) && n == sizeof(LINE))
    {
        ...
        WaitForSingleObject(td->sem, INFINITE);

        _tprintf(_T("Thread %d performing wait\n"), GetCurrentThreadId());
        ...
        if (!ReadFile(td->accountFile, &account, sizeof(LINE), &n, &ov) || n != sizeof(LINE))
        {
            ...
            ReleaseSemaphore(td->sem, 1, NULL);
            return -1;
        }
        ...
        _tprintf(_T("Thread %d performing signal\n"), GetCurrentThreadId());
...
INT _tmain(INT argc, LPTSTR argv[])
{
    ...
    if (sem == NULL)
    {
        _ftprintf(stderr, _T("Cannot create mutex.\n"));
        return -1;
    }
