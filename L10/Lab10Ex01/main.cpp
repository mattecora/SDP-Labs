...
DWORD Merger(DWORD nThreads, LPHANDLE threads, LPTHREAD_DATA threadData, LPTSTR outputFilename)
{
    DWORD i, lastLen, nWritten, term;
    ...
    while (nThreads > 0)
    {
        ...
        term = WaitForMultipleObjects(nThreads, threads, FALSE, INFINITE) - WAIT_OBJECT_0;

        // Allocate the partial results vector
        result = (LPDWORD)malloc((lastLen + threadData[term].length) * sizeof(DWORD));
        if (result == NULL)
        {
            _ftprintf(stderr, _T("Cannot allocate memory.\n"));
            return FALSE;
        ...
        Merge(lastResult, lastLen, threadData[term].vector, threadData[term].length, result);

        // Free old result and copy new one
        free(lastResult);
        lastResult = result;
        lastLen = lastLen + threadData[term].length;

        // Close thread handle and free associated vector
        CloseHandle(threads[term]);
        free(threadData[term].vector);

        // Remove thread and its data from the vectors
        for (i = term; i < nThreads - 1; i++)
        {
            threads[i] = threads[i + 1];
            threadData[i] = threadData[i + 1];
        }
        nThreads--;
    }
...
INT _tmain(INT argc, LPTSTR argv[])
{
    ...
    Merger(nThreads, threads, threadData, argv[argc - 1]);
